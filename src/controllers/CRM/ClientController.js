...
export default {
    ...
    get: async ({ query }, res) => {
        try {
            const data = await ClientService?.get(query);
    ...
    update: async ({ params, body }, res) => {
        try {
            const data = await ClientService?.update(params?.id, body);
    ...
    deleteMany: async ({ body }, res) => {
        try {
            const data = await ClientService?.deleteMany(body?.data);
    ...
    deleteAll: async ({ }, res) => {
        try {
            const data = await ClientService?.deleteAll();
            successMessage(res, `${data?.count} ligne(s) Supprimée(s) avec succès`)
        } catch (error) {
            errorMessage(res, error)
        }
