...
import EquipementService from "../../services/Inventaire/EquipementService.js";
...
export default {
    ...
    get: async ({ query }, res) => {
        try {
            const data = await EquipementService?.get(query);
    ...
    getList: async ({ }, res) => {
        try {
            const data = await EquipementService?.getList();
    ...
    getByID: async ({ params }, res) => {
        try {
            const data = await EquipementService?.getByID(params?.id);
    ...
    update: async ({ params, body }, res) => {
        try {
            const data = await EquipementService?.update(params?.id, body);
    ...
    delete: async ({ params }, res) => {
        try {
            await EquipementService?.delete(params?.id);
    ...
    deleteMany: async ({ body }, res) => {
        try {
            const data = await EquipementService?.deleteMany(body?.data);
    ...
    deleteAll: async ({ }, res) => {
        try {
            const data = await EquipementService?.deleteAll();
            successMessage(res, `${data?.count} ligne(s) Supprimée(s) avec succès`)
        } catch (error) {
            errorMessage(res, error)
        }
