...
import TypeAbonnementService from "../../services/Reabonnement/TypeAbonnementService.js";

export default {
    // create
    create: async ({ body }, res) => {
        try {
            const data = await TypeAbonnementService?.create(body);
    ...
    get: async ({ query }, res) => {
        try {
            const data = await TypeAbonnementService?.get(query);
    ...
    getList: async ({ }, res) => {
        try {
            const data = await TypeAbonnementService?.getList();
    ...
    getByID: async ({ params }, res) => {
        try {
            const data = await TypeAbonnementService?.getByID(params?.id);
    ...
    update: async ({ params, body }, res) => {
        try {
            const data = await TypeAbonnementService?.update(params?.id, body);
    ...
    delete: async ({ params }, res) => {
        try {
            await TypeAbonnementService?.delete(params?.id);
    ...
    deleteMany: async ({ body }, res) => {
        try {
            const data = await TypeAbonnementService?.deleteMany(body?.data);
    ...
    deleteAll: async ({ }, res) => {
        try {
            const data = await TypeAbonnementService?.deleteAll();
            successMessage(res, `${data?.count} ligne(s) Supprimée(s) avec succès`)
        } catch (error) {
            errorMessage(res, error)
        }
